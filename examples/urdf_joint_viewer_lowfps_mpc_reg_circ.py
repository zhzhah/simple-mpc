...
def main() -> None:
    ...
    vx_cmd_default = 0.0
    wz_cmd_default = 0.0
    if "CoMVelo_d_0" in df.columns:
        vx_cmd_default = float(df["CoMVelo_d_0"].to_numpy()[0])
    if "rpy_w_d_2" in df.columns:
        wz_cmd_default = float(df["rpy_w_d_2"].to_numpy()[0])
    vx_cmd_default = float(np.clip(vx_cmd_default, 0.0, 2.0))
    wz_cmd_default = float(np.clip(wz_cmd_default, 0.0, 3.0))
    v_cmd_slider = p.addUserDebugParameter("target_vx (m/s)", 0.0, 2.0, vx_cmd_default)
    wz_cmd_slider = p.addUserDebugParameter("target_wz (rad/s)", 0.0, 3.0, wz_cmd_default)
    ...
    def compute_mpc_inputs():
        ...
        vx_cmd_local = float(p.readUserDebugParameter(v_cmd_slider))
        wz_cmd_local = float(p.readUserDebugParameter(wz_cmd_slider))
    ...
    def build_arc_reference_states(vx_cmd_local: float, wz_cmd_local: float) -> list[np.ndarray]:
        refs = []
    ...
    def compute_icr_arc_params(vx_cmd_local: float, wz_cmd_local: float) -> np.ndarray:
    ...
    def process_frame():
        ...
        base_xyz, ball_xy, vx_cmd, wz_cmd, x_measured, v_world_cmd, q_term = compute_mpc_inputs()
        ...
        if problem_conf.get("icr_arc_cstr", False):
            icr_params = compute_icr_arc_params(vx_cmd, wz_cmd)
            try:
        ...
        x_term[nq:] = 0.0
        if ENABLE_ICR_ARC_REF:
            refs = build_arc_reference_states(vx_cmd, wz_cmd)
    ...
    while True:
        ...
        vx_cmd = float(p.readUserDebugParameter(v_cmd_slider))
        wz_cmd = float(p.readUserDebugParameter(wz_cmd_slider))
        cost_vals = []
        all_slider_keys = sorted(list(cost_sliders.keys()) + list(jointpos_sliders.keys()))
        for key in all_slider_keys:
            if key in cost_sliders:
