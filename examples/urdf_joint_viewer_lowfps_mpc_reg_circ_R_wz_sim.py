...
def build_mpc():
    ...
    if ENABLE_ICR_ARC_REF:
        ...
        w_basepos = [1, 1, 1, 0.0, 1000.0, 1000]
    ...
    w_basepos_T = [100000, 100000, 10000, 10000.0, 10000.0, 100000]
    w_jointpos_T = [10.1, 10.1, 10.1, 0.0]
...
def rpy_from_quat(quat_xyzw: np.ndarray) -> tuple[float, float, float]:
...
def main() -> None:
    ...
    def _add_weight_slider(label: str, value: float, vmax: float) -> int:
        v0 = float(value)
        v1 = float(max(vmax, v0 * 2.0, 1.0))
        return p.addUserDebugParameter(label, 0.0, v1, v0)
    ...
    dt_mpc = float(problem_conf["timestep"])
    N_simu = 10
    dt_simu = dt_mpc / max(N_simu, 1)
    ...
    if problem_conf.get("use_vector_glide_cost", False):
        cost_sliders["vector_glide_weight_base"] = _add_weight_slider(
            "vector_glide_weight_base",
            float(problem_conf.get("vector_glide_weight_base", 1000.0)),
            5000.0,
        )
    # Track width (soft cost)
    if problem_conf.get("track_width_cstr", False) and problem_conf.get("w_track_width", 0.0) > 0.0:
        cost_sliders["w_track_width"] = _add_weight_slider(
            "w_track_width",
            float(problem_conf.get("w_track_width", 0.0)),
            10000.0,
        )
    # Foot sum (soft cost)
    if problem_conf.get("foot_sum_cstr", False) and problem_conf.get("w_foot_sum", 0.0) > 0.0:
        cost_sliders["w_foot_sum"] = _add_weight_slider(
            "w_foot_sum",
            float(problem_conf.get("w_foot_sum", 0.0)),
            10000.0,
        )
    ...
    if problem_conf.get("min_wheel_distance_cost", False) and problem_conf.get("w_min_wheel_distance", 0.0) > 0.0:
        cost_sliders["w_min_wheel_distance"] = _add_weight_slider(
            "w_min_wheel_distance",
            float(problem_conf.get("w_min_wheel_distance", 0.0)),
            1000.0,
        )
    ...
    if problem_conf.get("force_z_variance_cost", False) and problem_conf.get("w_force_z_variance", 0.0) > 0.0:
        cost_sliders["w_force_z_variance"] = _add_weight_slider(
            "w_force_z_variance",
            float(problem_conf.get("w_force_z_variance", 0.0)),
            1000.0,
        )
    ...
    if problem_conf.get("joint_limit_soft_cost", False) and problem_conf.get("w_joint_limit_soft", 0.0) > 0.0:
        cost_sliders["w_joint_limit_soft"] = _add_weight_slider(
            "w_joint_limit_soft",
            float(problem_conf.get("w_joint_limit_soft", 0.0)),
            1000.0,
        )
    # Hip sum (soft cost)
    if problem_conf.get("w_hip_sum", 0.0) > 0.0 and problem_conf.get("hip_joint_names", []):
        cost_sliders["w_hip_sum"] = _add_weight_slider(
            "w_hip_sum",
            float(problem_conf.get("w_hip_sum", 0.0)),
            1000.0,
        )
    # ICR axle line (soft cost)
    if problem_conf.get("w_icr_axle", 0.0) > 0.0:
        cost_sliders["w_icr_axle"] = _add_weight_slider(
            "w_icr_axle",
            float(problem_conf.get("w_icr_axle", 0.0)),
            1000.0,
        )
    # Soft constraints (if enabled)
    if problem_conf.get("soft_constraints", False):
        if problem_conf.get("w_soft_contact_vel", 0.0) > 0.0:
            cost_sliders["w_soft_contact_vel"] = _add_weight_slider(
                "w_soft_contact_vel",
                float(problem_conf.get("w_soft_contact_vel", 0.0)),
                200.0,
            )
        if problem_conf.get("w_soft_friction", 0.0) > 0.0:
            cost_sliders["w_soft_friction"] = _add_weight_slider(
                "w_soft_friction",
                float(problem_conf.get("w_soft_friction", 0.0)),
                200.0,
            )
        if problem_conf.get("w_soft_land", 0.0) > 0.0:
            cost_sliders["w_soft_land"] = _add_weight_slider(
                "w_soft_land",
                float(problem_conf.get("w_soft_land", 0.0)),
                200.0,
            )
    ...
    def compute_mpc_inputs(q_meas_local: np.ndarray, v_meas_local: np.ndarray):
        base_xyz_local = q_meas_local[:3].copy()
        roll_local, pitch_local, yaw_local = rpy_from_quat(q_meas_local[3:7])
        ...
        if r_cmd_local <= 1e-9:
            ...
            wz_cmd_local = min(3.0, vx_cmd_local / r_cmd_local)
        ...
        if r_cmd_local <= 1e-9:
            ...
            dx_local = r_cmd_local * np.sin(dtheta_local)
            dy_local = r_cmd_local * (1.0 - np.cos(dtheta_local))
        cy_local = np.cos(yaw_local)
        sy_local = np.sin(yaw_local)
        delta_world_local = np.array(
        ...
        v_world_cmd_local[0] = vx_cmd_local * np.cos(yaw_local)
        v_world_cmd_local[1] = vx_cmd_local * np.sin(yaw_local)
        v_world_cmd_local[5] = wz_cmd_local

        q_term_local = model_handler.getReferenceState()[:nq].copy()
        q_term_local[0] = ball_xy_local[0]
        q_term_local[1] = ball_xy_local[1]
        q_term_local[2] = base_xyz_local[2]
        yaw_term_local = yaw_local + wz_cmd_local * TARGET_INTEGRATION_T
        q_term_local[3:7] = quat_from_rpy(roll_local, pitch_local, yaw_term_local)
        q_term_local[7:] = q_meas_local[7:].copy()
    ...
    def build_arc_reference_states(
        ...
        for t_idx in range(horizon_T):
            ...
            q_ref[0] = base_xyz_local[0] + dx_world
            q_ref[1] = base_xyz_local[1] + dy_world
            q_ref[2] = base_xyz_local[2]
            yaw_ref = yaw_local + wz_cmd_local * t
            q_ref[3:7] = quat_from_rpy(roll_local, pitch_local, yaw_ref)
    ...
    def compute_icr_arc_params(r_cmd_local: float, wz_cmd_local: float, yaw_local: float) -> np.ndarray:
    ...
    def process_frame(q_meas_local: np.ndarray, v_meas_local: np.ndarray):
        ...
        (
            ...
            roll_local,
            pitch_local,
        ) = compute_mpc_inputs(q_meas_local, v_meas_local)
        ...
        for key, slider in cost_sliders.items():
            problem_conf[key] = float(p.readUserDebugParameter(slider))
        ...
        if problem_conf.get("icr_arc_cstr", False):
            icr_params = compute_icr_arc_params(r_cmd, wz_cmd, yaw_local)
            try:
        ...
        x_term[nq:] = 0.0
        if ENABLE_ICR_ARC_REF:
            refs = build_arc_reference_states(base_xyz, roll_local, pitch_local, yaw_local, r_cmd, wz_cmd)
        ...
        return mpc, v_world_cmd, x_measured
    ...
    while True:
        q_meas, v_meas = device.measureState()
        if "_handle_mouse_camera" in globals():
            _handle_mouse_camera()
        r_cmd = float(p.readUserDebugParameter(r_cmd_slider))
        if r_cmd <= 1e-9:
            wz_cmd = 3.0
        else:
            wz_cmd = min(3.0, 1.0 / r_cmd)
        cost_vals = []
        for key in sorted(cost_sliders.keys()):
            cost_vals.append(round(float(p.readUserDebugParameter(cost_sliders[key])), 6))
        cmd = (round(r_cmd, 6), round(wz_cmd, 6), *cost_vals)
        if cmd != last_cmd:
            last_cmd = cmd
        mpc, v_world_cmd, x_measured = process_frame(q_meas, v_meas)

        if len(mpc.xs) < 2 or len(mpc.us) < 2:
            q_meas, v_meas = device.measureState()
            x_measured = np.concatenate([q_meas, v_meas])
            time.sleep(dt_mpc)
            continue
        ...
        for sub_step in range(N_simu):
            ...
            # Use MPC only for joint posture, base uses measurement and cmd velocity
            q_interp[:7] = q_meas[:7]
