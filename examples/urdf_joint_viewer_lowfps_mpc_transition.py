    ...
    def process_frame(frame: int):
        ...
        if SHOW_MPC_STATE_LINE and mpc.xs:
            ...
            for idx in idxs:
                xs = mpc.xs[int(idx)]
                pts.append([
                    xs[0] + base_xyz[0] + TRAJ_POS_OFFSET[0],
                    xs[1] + base_xyz[1] + TRAJ_POS_OFFSET[1],
                    xs[2] + TRAJ_POS_OFFSET[2],
                ])
        ...
        if SHOW_MPC_FOOT_LINES and mpc.xs:
            ...
            for idx in idxs:
                ...
                for name in wheel_link_names:
                    ...
                    if FIX_FOOT_Z and foot_axis_z_world:
                        z_fixed = foot_axis_z_world.get(name, float(pw[2]))
                        p_vis = [pw[0] + TRAJ_POS_OFFSET[0], pw[1] + TRAJ_POS_OFFSET[1], z_fixed + TRAJ_POS_OFFSET[2]]
                    else:
                        p_vis = [pw[0] + TRAJ_POS_OFFSET[0], pw[1] + TRAJ_POS_OFFSET[1], pw[2] + TRAJ_POS_OFFSET[2]]
                    prev = prev_pts[name]
                    if prev is not None:
                        if use_geom_lines:
