...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--repro-radius", type=float, default=None, help="Override target radius (m) for repro")
    parser.add_argument("--repro-weight", type=float, default=None, help="Override vector_glide_weight_base for repro")
    parser.add_argument("--repro-use-vector-glide", action="store_true", help="Force use_vector_glide_cost=True")
    parser.add_argument("--repro-no-vector-glide", action="store_true", help="Force use_vector_glide_cost=False")
    parser.add_argument("--repro-once", action="store_true", help="Run one frame and exit (for repro)")
    parser.add_argument(
        "--repro-zero-omega-at-zero-radius",
        action="store_true",
        default=False,
        help="When repro radius=0, force omega=0 to match data-gen behavior",
    )
...
def main() -> None:
    args = parse_args()
    if args.repro_use_vector_glide and args.repro_no_vector_glide:
        raise SystemExit("Use only one of --repro-use-vector-glide or --repro-no-vector-glide")
    repro_use_vg = None
    if args.repro_use_vector_glide:
        repro_use_vg = True
    elif args.repro_no_vector_glide:
        repro_use_vg = False
    repro_radius = args.repro_radius
    repro_weight = args.repro_weight
    repro_mode = any(
        v is not None for v in (repro_radius, repro_weight, repro_use_vg)
    ) or args.repro_once
    ...
    # Single ghost for MPC process reference (same target as terminal)
    mpc_process_ref_ghost = None
    if SHOW_MPC_PROCESS_REF_GHOST:
        ghost_id, ghost_joints = spawn_ghost(
            FIXED_URDF, [0.2, 0.7, 1.0, MPC_GHOST_ALPHA], model_names
        )
        mpc_process_ref_ghost = (ghost_id, ghost_joints)
    ...
    def compute_mpc_inputs():
        base_xyz_local = base_xyz0
        if repro_radius is not None:
            r_cmd_local = float(repro_radius)
        else:
            r_cmd_local = float(p.readUserDebugParameter(r_cmd_slider))
        r_cmd_local = float(np.clip(r_cmd_local, r_min, r_max))
        vx_cmd_local = 2.0
        if r_cmd_local <= 1e-9:
            if repro_mode and args.repro_zero_omega_at_zero_radius:
                wz_cmd_local = 0.0
            else:
                wz_cmd_local = 3.0
        else:
            wz_cmd_local = vx_cmd_local / r_cmd_local
            if wz_cmd_local > 3.0:
                wz_cmd_local = 3.0
        ...
        v_world_cmd_local[0] = vx_cmd_local * np.cos(yaw0)
        v_world_cmd_local[1] = vx_cmd_local * np.sin(yaw0)
        ...
        yaw_term_local = yaw0 + wz_cmd_local * TARGET_INTEGRATION_T
    ...
    def build_arc_reference_states(r_cmd_local: float, wz_cmd_local: float) -> list[np.ndarray]:
        refs = []
    ...
    def process_frame():
        ...
        if repro_weight is not None:
            problem_conf["vector_glide_weight_base"] = float(repro_weight)
            problem_conf["vector_glide_weight"] = float(repro_weight)
        if repro_use_vg is not None:
            problem_conf["use_vector_glide_cost"] = bool(repro_use_vg)
        # Adapt vector glide weight based on twist direction vs. foot alignment
        if problem_conf.get("use_vector_glide_cost", False) and not repro_mode:
        ...
        try:
            ...
            print("[REPRO] create_mpc_instance failed:", exc)
            print(
                "  r_cmd={:.6g} vx_cmd={:.6g} wz_cmd={:.6g} weight={:.6g} use_vg={} repro_mode={}".format(
                    r_cmd,
                    vx_cmd,
                    wz_cmd,
                    float(problem_conf.get("vector_glide_weight_base", 0.0)),
                    bool(problem_conf.get("use_vector_glide_cost", False)),
                    repro_mode,
                )
            )
            raise
        ...
        if problem_conf.get("icr_arc_cstr", False):
            icr_params = compute_icr_arc_params(r_cmd, wz_cmd)
            try:
        ...
        x_term[nq:] = 0.0
        if ENABLE_ICR_ARC_REF:
            refs = build_arc_reference_states(r_cmd, wz_cmd)
        ...
        try:
            ...
            print("[REPRO] mpc.iterate failed:", exc)
            print(
                "  r_cmd={:.6g} vx_cmd={:.6g} wz_cmd={:.6g} weight={:.6g} use_vg={} repro_mode={}".format(
                    r_cmd,
                    vx_cmd,
        ...
        if PRINT_MPC_DEBUG:
            print("=" * 80)
            print(f"[MPC] frame={frame0} horizon_steps={len(mpc.xs) - 1}")
        ...
        if SHOW_MPC_PROCESS_REF_GHOST and mpc_process_ref_ghost is not None:
            ghost_id, ghost_joints = mpc_process_ref_ghost
            try:
            ...
            # Display process reference offset to current robot position
