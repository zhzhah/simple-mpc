...
SHOW_MPC_PROCESS_REF_GHOST = True
...
# Enforce base xy to follow the ICR arc (soft constraint via stage reference + weight)
...
# Fixed pitch stats from /home/yzc/MPCtest/traj/processed_low.csv
PITCH_MEAN = 0.2966951191391238
...
def _solver_iteration_count(mpc) -> int | None:
    try:
        solver = mpc.solver
    except Exception:
        return None
    ...
    try:
        if hasattr(solver, "results"):
            res = solver.results
        elif hasattr(solver, "results_"):
            res = solver.results_
    except Exception:
        res = None
    for name in ("iter", "n_iter", "niter", "num_iters", "numIterations", "iterations", "niters"):
        if res is not None and hasattr(res, name):
            try:
                return int(getattr(res, name))
            except Exception:
                pass
    for name in ("getNumIterations", "getIterations", "num_iters", "iterations"):
...
def debug_mpc_horizon(mpc, model_handler, problem_conf, wheel_link_names, base_height, icr_params=None):
    model = model_handler.getModel()
    nq = model.nq
    nv = model.nv
    ...
    for t in range(len(mpc.xs) - 1):
        ...
        hip_sum_cost = 0.0
        hip_sum_res = None
        hip_joint_names = problem_conf.get("hip_joint_names", [])
        if problem_conf.get("w_hip_sum", 0.0) > 0.0 and hip_joint_names:
            hip_sum = 0.0
            for jn in hip_joint_names:
                ...
                hip_sum += float(np.sum(q0[idx_q : idx_q + nq_j]))
            hip_sum_res = hip_sum
            hip_sum_cost = float(problem_conf.get("w_hip_sum", 0.0) * (hip_sum * hip_sum))

        icr_axle_cost = 0.0
        icr_axle_res = None
        if problem_conf.get("w_icr_axle", 0.0) > 0.0 and icr_params is not None:
            if icr_params[3] >= 0.5:
                cx, cy = float(icr_params[0]), float(icr_params[1])
                icr = np.array([cx, cy], dtype=float)
                data_axle = pin.Data(model)
                pin.forwardKinematics(model, data_axle, x0[:nq], x0[nq:])
                pin.updateFramePlacements(model, data_axle)
                fl_id = mpc.getModelHandler().getFootFrameId(mpc.getModelHandler().getFootNb("FL_wheel"))
                fr_id = mpc.getModelHandler().getFootFrameId(mpc.getModelHandler().getFootNb("FR_wheel"))
                rl_id = mpc.getModelHandler().getFootFrameId(mpc.getModelHandler().getFootNb("RL_wheel"))
                rr_id = mpc.getModelHandler().getFootFrameId(mpc.getModelHandler().getFootNb("RR_wheel"))
                fl = data_axle.oMf[fl_id].translation[:2]
                rl = data_axle.oMf[rl_id].translation[:2]
                fr = data_axle.oMf[fr_id].translation[:2]
                rr = data_axle.oMf[rr_id].translation[:2]
                ...
                def dist_point_line(p, a, b):
                    ...
                    if denom < 1e-9:
                        return 0.0
                    num = abs((p[0] - a[0]) * ab[1] - (p[1] - a[1]) * ab[0])
                    return float(num / denom)

                d_left = dist_point_line(icr, fl, rl)
                d_right = dist_point_line(icr, fr, rr)
                icr_axle_res = [d_left, d_right]
                icr_axle_cost = float(problem_conf.get("w_icr_axle", 0.0) * (d_left * d_left + d_right * d_right))
        ...
        row = {
            ...
            "hip_sum_cost": hip_sum_cost,
            "icr_axle_cost": icr_axle_cost,
        }
        ...
        if hip_sum_res is not None:
            row["hip_sum_res"] = hip_sum_res
        if icr_axle_res is not None:
            row["icr_axle_res"] = icr_axle_res
        if contact_states:
        ...
        for k, v in row.items():
            if not k.endswith("_cost"):
                continue
            if k in {"state_cost", "control_cost", "foot_cost"}:
                continue
            extra_costs.append((k, float(v)))
        extra_costs.sort(key=lambda kv: kv[0])
        if extra_costs:
            parts = ["{}={:.6g}".format(k, v) for k, v in extra_costs]
            print("  " + " ".join(parts))
        if contact_states:
...
def main() -> None:
    ...
    r_cmd_default = 1.0
    if "rpy_w_d_2" in df.columns:
        wz_cmd_default = float(df["rpy_w_d_2"].to_numpy()[0])
        if abs(wz_cmd_default) > 1e-6:
            r_cmd_default = 1.0 / abs(wz_cmd_default)
    r_min = 0.0
    r_max = 2.0
    r_cmd_default = float(np.clip(r_cmd_default, r_min, r_max))
    r_cmd_slider = p.addUserDebugParameter("target_R (m)", r_min, r_max, r_cmd_default)
    ...
    ball_vis = None
    target_ball_ids: list[int] = []
    if SHOW_TARGET_BALL:
        ball_vis = p.createVisualShape(p.GEOM_SPHERE, radius=TARGET_BALL_RADIUS, rgbaColor=[1.0, 0.2, 0.2, 1])

    def _ensure_target_balls(count: int):
        if not SHOW_TARGET_BALL or ball_vis is None:
            return
        while len(target_ball_ids) < count:
            target_ball_ids.append(p.createMultiBody(baseMass=0, baseVisualShapeIndex=ball_vis))
    icr_id = None
    if SHOW_ICR_POINT:
    ...
    def _exp20_scale(s: float) -> float:
    ...
    def _handle_mouse_camera() -> None:
        mouse_button_event = getattr(p, "MOUSE_BUTTON_EVENT", 2)
        mouse_move_event = getattr(p, "MOUSE_MOVE_EVENT", 1)
        left_button = getattr(p, "B3G_LEFT_MOUSE_BUTTON", 0)
        right_button = getattr(p, "B3G_RIGHT_MOUSE_BUTTON", 1)
        middle_button = getattr(p, "B3G_MIDDLE_MOUSE_BUTTON", 2)
        ...
        for ev in events:
            if len(ev) < 5:
                continue
            etype, mx, my, button, state = ev[0], ev[1], ev[2], ev[3], ev[4]
            if etype == mouse_button_event:
                if state == p.KEY_IS_DOWN:
                    mouse_state["buttons"].add(button)
                    mouse_state["last"] = (mx, my)
                elif state == p.KEY_WAS_RELEASED:
                    mouse_state["buttons"].discard(button)
                    mouse_state["last"] = (mx, my)
            elif etype == mouse_move_event:
                if mouse_state["last"] is None or not mouse_state["buttons"]:
                    mouse_state["last"] = (mx, my)
                    continue
                dx = float(mx - mouse_state["last"][0])
                dy = float(my - mouse_state["last"][1])
                cam = p.getDebugVisualizerCamera()
                yaw = float(cam[8])
                pitch = float(cam[9])
                dist = float(cam[10])
                target = np.array(cam[11], dtype=float)
                rotate_buttons = {left_button, right_button, 0, 1}
                pan_buttons = {middle_button, 2}
                is_rotate = bool(mouse_state["buttons"] & rotate_buttons)
                is_pan = bool(mouse_state["buttons"] & pan_buttons)
                if not is_rotate and not is_pan:
                    # Fallback when button codes are unknown
                    is_rotate = True
                    if len(mouse_state["buttons"]) > 1:
                        is_pan = True
                if is_rotate:
                    yaw += dx * rotate_sens
                    pitch = float(np.clip(pitch - dy * rotate_sens, -89.0, 89.0))
                if is_pan:
                    ...
                    target += (-dx * pan_sens * dist) * right + (dy * pan_sens * dist) * up
                p.resetDebugVisualizerCamera(dist, yaw, pitch, target.tolist())
                mouse_state["last"] = (mx, my)
    ...
    for i in range(3):
        base = float(w_jointpos_base[i]) if i < len(w_jointpos_base) else 0.0
        jointpos_sliders[f"w_jointpos_{i}"] = (base, _add_weight_slider(f"w_jointpos_{i}"))

    cost_text_ids = {}
    cost_text_keys = sorted(list(cost_sliders.keys()) + list(jointpos_sliders.keys()))
    cost_text_origin = np.array([-1.0, 0.0, 1.35], dtype=float)
    cost_text_step = np.array([0.0, 0.0, -0.06], dtype=float)
    ...
    q_init[0] = base_xyz0[0]
    q_init[1] = base_xyz0[1]
    ...
    for name in wheel_link_names:
        fid = model_handler.getFootFrameId(model_handler.getFootNb(name))
        foot_start_world[name] = data_init.oMf[fid].translation.copy()
    ...
    foot_traj_offset = {name: None for name in wheel_link_names}
    com_traj_lines = []
    com_traj_prev = None
    mpc_state_line_ids = []
    ...
    mpc_terminal_ghosts: list[tuple[int, list[int]]] = []
    mpc_ref_ghosts: list[tuple[int, list[int]]] = []
    mpc_process_ref_ghosts: list[tuple[int, list[int]]] = []

    def _ensure_ghosts(ghosts: list[tuple[int, list[int]]], count: int, color: list[float]):
        while len(ghosts) < count:
            ghost_id, ghost_joints = spawn_ghost(FIXED_URDF, color, model_names)
            ghosts.append((ghost_id, ghost_joints))
    ...
    def _arc_displacement(r_cmd_local: float, theta: float, yaw_base: float) -> tuple[float, float]:
        if r_cmd_local <= 1e-9 or abs(theta) < 1e-12:
            dx_local = 0.0
            dy_local = 0.0
        else:
            R_local = r_cmd_local
            dx_local = R_local * np.sin(theta)
            dy_local = R_local * (1.0 - np.cos(theta))
        cy_local = np.cos(yaw_base)
        sy_local = np.sin(yaw_base)
        dx_world = cy_local * dx_local - sy_local * dy_local
        dy_world = sy_local * dx_local + cy_local * dy_local
        return dx_world, dy_world

    def compute_segment_plan(r_cmd_local: float) -> list[dict]:
        r_cmd_local = float(np.clip(r_cmd_local, r_min, r_max))
        if r_cmd_local <= 1e-9:
            max_angle = WZ_MAX * TARGET_INTEGRATION_T
        else:
            max_angle = min(WZ_MAX, V_MAX / r_cmd_local) * TARGET_INTEGRATION_T
        max_angle = max(max_angle, 1e-9)

        if max_angle >= np.pi:
            num_segments = 1
        else:
            num_segments = int(np.ceil(np.pi / max_angle))
        angle_each = np.pi / num_segments

        segments = []
        theta_acc = 0.0
        for _ in range(num_segments):
            theta_start = theta_acc
            theta_end = theta_start + angle_each
            w_cmd = angle_each / TARGET_INTEGRATION_T
            if r_cmd_local <= 1e-9:
                v_cmd = 0.0
            else:
                v_cmd = min(V_MAX, w_cmd * r_cmd_local)
            segments.append(
                {
                    ...
                    "v_cmd": float(v_cmd),
                    "w_cmd": float(w_cmd),
                    "angle": float(angle_each),
                }
            )
            theta_acc = theta_end
        return segments

    def compute_segment_inputs(
        r_cmd_local: float,
        v_cmd_local: float,
        w_cmd_local: float,
        theta_start: float,
        theta_end: float,
        x_measured_override: np.ndarray | None,
    ):
        base_xyz_local = base_xyz0

        pitch_use = pitch0
        if x_measured_override is None:
            ...
            q_meas_local[3:7] = quat_from_rpy(roll0, pitch_use, yaw0)
            for joint_name, qv in zip(joint_order, joints0):
                joint_id = model.getJointId(joint_name)
                idx_q = model.joints[joint_id].idx_q
            ...
            if "vel_world_x" in df.columns and "vel_world_y" in df.columns:
                v_meas_local[0] = float(df["vel_world_x"].to_numpy()[frame0])
                v_meas_local[1] = float(df["vel_world_y"].to_numpy()[frame0])
                v_meas_local[2] = float(df["CoMVelo_2"].to_numpy()[frame0]) if "CoMVelo_2" in df.columns else 0.0
            if "CoMAngleVelo_W_0" in df.columns:
                v_meas_local[3] = float(df["CoMAngleVelo_W_0"].to_numpy()[frame0])
                v_meas_local[4] = float(df["CoMAngleVelo_W_1"].to_numpy()[frame0])
                v_meas_local[5] = float(df["CoMAngleVelo_W_2"].to_numpy()[frame0])
            x_measured_local = np.concatenate([q_meas_local, v_meas_local])
        else:
            x_measured_local = np.asarray(x_measured_override, dtype=float).copy()

        yaw_start = yaw0 + theta_start
        v_world_cmd_local = np.zeros(6)
        v_world_cmd_local[0] = v_cmd_local * np.cos(yaw_start)
        v_world_cmd_local[1] = v_cmd_local * np.sin(yaw_start)
        v_world_cmd_local[5] = w_cmd_local

        dx_world, dy_world = _arc_displacement(r_cmd_local, theta_end, yaw0)
        ball_xy_local = base_xyz_local[:2] + np.array([dx_world, dy_world])

        q_term_local = model_handler.getReferenceState()[:nq].copy()
        q_term_local[0] = dx_world
        q_term_local[1] = dy_world
        q_term_local[2] = base_xyz_local[2]
        yaw_term_local = yaw0 + theta_end
        q_term_local[3:7] = quat_from_rpy(roll0, pitch_use, yaw_term_local)
        for joint_name, qv in zip(joint_order, joints0):
            joint_id = model.getJointId(joint_name)
            idx_q = model.joints[joint_id].idx_q
    ...
    def build_arc_reference_states(r_cmd_local: float, wz_cmd_local: float, theta_start: float) -> list[np.ndarray]:
        refs = []
        dt = float(problem_conf["timestep"])
        for t_idx in range(horizon_T):
            t = t_idx * dt
            theta = theta_start + wz_cmd_local * t
            dx_world, dy_world = _arc_displacement(r_cmd_local, theta, yaw0)
    ...
    def compute_icr_arc_params(r_cmd_local: float, wz_cmd_local: float) -> np.ndarray:
        min_omega = float(problem_conf.get("icr_arc_min_omega", 1e-6))
        if abs(wz_cmd_local) < min_omega:
    ...
    def process_frame():
        ...
        quat = quat_from_rpy(roll0, pitch0, yaw0)
        ...
        if SHOW_TARGET_GHOST and target_ghost_id is not None and tgt_world_xyz is not None:
            tgt_pos = [
                tgt_world_xyz[frame0][0] + BASE_POS_OFFSET[0],
                tgt_world_xyz[frame0][1] + BASE_POS_OFFSET[1],
                tgt_world_xyz[frame0][2] + BASE_POS_OFFSET[2],
            ]
            p.resetBasePositionAndOrientation(target_ghost_id, tgt_pos, [0, 0, 0, 1])

        r_cmd = float(p.readUserDebugParameter(r_cmd_slider))
        r_cmd = float(np.clip(r_cmd, r_min, r_max))
        segments = compute_segment_plan(r_cmd)
        if not segments:
            return

        _ensure_target_balls(len(segments))

        base_xyz = base_xyz0

        # Joints
        for idx, qv in zip(joint_indices, joints0):
            p.resetJointState(body_id, idx, float(qv))

        if SHOW_ICR_POINT and icr_id is not None:
            # Use first-segment command for ICR visualization.
            first_seg = segments[0]
            yaw_start = yaw0 + first_seg["theta_start"]
            v_world_cmd_first = np.zeros(6)
            v_world_cmd_first[0] = first_seg["v_cmd"] * np.cos(yaw_start)
            v_world_cmd_first[1] = first_seg["v_cmd"] * np.sin(yaw_start)
            v_world_cmd_first[5] = first_seg["w_cmd"]
            omega = float(v_world_cmd_first[5])
            if abs(omega) > 1e-6:
                icr_x = base_xyz[0] - v_world_cmd_first[1] / omega
                icr_y = base_xyz[1] + v_world_cmd_first[0] / omega
        ...
        if SHOW_VELOCITY_ARROWS:
            ...
            if vel_world is not None:
                end = [base_xyz[0] + vel_world[frame0][0] * VEL_SCALE, base_xyz[1] + vel_world[frame0][1] * VEL_SCALE, base_z]
            ...
            if vel_d_world is not None:
                end = [base_xyz[0] + vel_d_world[frame0][0] * VEL_SCALE, base_xyz[1] + vel_d_world[frame0][1] * VEL_SCALE, base_z + 0.02]
            ...
            if wz is not None:
                end = [base_xyz[0], base_xyz[1], base_z + 0.05 + wz[frame0] * ANG_VEL_SCALE]
            ...
            if wz_d is not None:
                end = [base_xyz[0], base_xyz[1], base_z + 0.07 + wz_d[frame0] * ANG_VEL_SCALE]
        ...
        for key, (base, slider, mode) in cost_sliders.items():
            ...
            if key in cost_text_ids:
                text = f"{key} = {weight:.6g}"
                pos = (cost_text_origin + cost_text_keys.index(key) * cost_text_step).tolist()
                cost_text_ids[key] = p.addUserDebugText(
                    text,
                    pos,
                    textColorRGB=cost_text_color,
                    textSize=1.2,
                    replaceItemUniqueId=cost_text_ids[key],
                )
        w_jointpos_base = problem_conf.get("w_jointpos_base", [0.0, 0.0, 0.0, 0.0])
        w_jointpos = list(w_jointpos_base[:4])
        for i in range(3):
            key = f"w_jointpos_{i}"
        ...
        if len(w_jointpos) < 4:
            w_jointpos = (w_jointpos + [0.0, 0.0, 0.0, 0.0])[:4]
        problem_conf["w_jointpos"] = w_jointpos
        problem_conf["w_jointpos_T"] = list(w_jointpos)
        w_x = problem_conf.get("w_x", None)
        w_x_T = problem_conf.get("w_x_terminal", None)
        if w_x is not None and w_x_T is not None:
            n_joints = max(nv - 6, 0)
            if n_joints > 0:
                w_q_joint = np.resize(np.tile(w_jointpos, 4), n_joints)
                for j in range(n_joints):
                    idx = 6 + j
                    w_x[idx, idx] = w_q_joint[j]
                    w_x_T[idx, idx] = w_q_joint[j]
        for key in ("w_x", "w_x_terminal", "w_u", "w_cent", "w_centder", "w_cent_terminal"):
            mat = problem_conf.get(key)
            if mat is not None and not np.all(np.isfinite(mat)):
                problem_conf[key] = np.nan_to_num(mat, nan=0.0, posinf=0.0, neginf=0.0)
        segment_results = []
        x_measured = None
        x_measured0 = None
        for seg_idx, seg in enumerate(segments):
            ...
            if seg_idx == 0:
                x_measured0 = x_measured_seg

            # Adapt vector glide weight based on twist direction vs. foot alignment
            if problem_conf.get("use_vector_glide_cost", False):
                update_vector_glide_weight(problem_conf, model_handler, x_measured_seg, v_world_cmd[5], wheel_link_names)

            # Fresh MPC instance each segment to reset initial guess
            ...
            if problem_conf.get("icr_arc_cstr", False):
                icr_params = compute_icr_arc_params(r_cmd, v_world_cmd[5])
                try:
                    mpc.ocp_handler.setIcrArcParams(icr_params)
                except Exception:
                    pass

            # Terminal/process target: target position from local integration
            x_term = model_handler.getReferenceState().copy()
            x_term[:nq] = q_term
            x_term[nq:] = 0.0
            if ENABLE_ICR_ARC_REF:
                refs = build_arc_reference_states(r_cmd, v_world_cmd[5], seg["theta_start"])
            ...
            reset_mpc_initial_guess(mpc, x_measured_seg)
            solve_start = time.perf_counter()
            mpc.iterate(x_measured_seg)
            solve_time = time.perf_counter() - solve_start
            solve_iters = _solver_iteration_count(mpc)

            segment_results.append(
                {
                    "mpc": mpc,
                    "q_term": q_term,
                    "v_world_cmd": v_world_cmd,
                    "x_measured": x_measured_seg,
                    "solve_time": solve_time,
                    "solve_iters": solve_iters,
                    "theta_start": seg["theta_start"],
                }
            )
            if mpc.xs:
                x_measured = np.asarray(mpc.xs[-1]).copy()
        if x_measured0 is not None:
            x_measured = x_measured0
        ...
        if SHOW_FOOT_TRAJECTORY or SHOW_COM_TRAJECTORY:
            ...
            if SHOW_FOOT_TRAJECTORY:
                ...
                for name in wheel_link_names:
                    fid = model_handler.getFootFrameId(model_handler.getFootNb(name))
                    p_now = data_traj.oMf[fid].translation.copy()
                    if foot_traj_offset[name] is None:
                        foot_traj_offset[name] = foot_start_world[name] - p_now
                    p_now = p_now + foot_traj_offset[name]
        ...
        if SHOW_MPC_STATE_LINE and segment_results:
            if use_geom_lines:
        ...
        if SHOW_MPC_FOOT_LINES and segment_results:
            foot_colors = {
                "FL_wheel": [1.0, 0.0, 0.0],  # red
                "FR_wheel": [0.0, 1.0, 0.0],  # green
                "RL_wheel": [0.0, 0.4, 1.0],  # blue
            ...
            for seg_res in segment_results:
                ...
                for idx in idxs:
                    ...
                    for name in wheel_link_names:
                        ...
                        p_vis = [pw[0] + BASE_POS_OFFSET[0], pw[1] + BASE_POS_OFFSET[1], pw[2] + BASE_POS_OFFSET[2]]
                        prev = prev_pts[name]
                        if prev is not None:
                            if use_geom_lines:
        ...
        for seg_idx, seg_res in enumerate(segment_results):
            mpc = seg_res["mpc"]
            q_term = seg_res["q_term"]
            v_world_cmd = seg_res["v_world_cmd"]
            solve_time = seg_res["solve_time"]
            solve_iters = seg_res["solve_iters"]
            rows = None
            icr_params = compute_icr_arc_params(r_cmd, v_world_cmd[5])
            if PRINT_MPC_DEBUG:
                print("=" * 80)
                print(f"[MPC] frame={frame0} segment={seg_idx + 1}/{len(segment_results)} horizon_steps={len(mpc.xs) - 1 if mpc.xs else 0}")
                print(f"  base_xyz={base_xyz}")
                print(f"  v_world_cmd={v_world_cmd}")
                print(f"  terminal_q={q_term}")
                rows = debug_mpc_horizon(mpc, model_handler, problem_conf, wheel_link_names, base_xyz[2], icr_params)
                weight_rows = []
                ...
                rows = debug_mpc_horizon(mpc, model_handler, problem_conf, wheel_link_names, base_xyz[2], icr_params)
            if mpc.xs:
                x_actual = np.asarray(mpc.xs[-1])
                desired_pos_xy = np.array([q_term[0], q_term[1]], dtype=float)
                desired_yaw = yaw_from_quat(q_term[3:7])
                desired_vel_xy = np.array([v_world_cmd[0], v_world_cmd[1]], dtype=float)
                desired_yaw_rate = float(v_world_cmd[5])
                actual_pos_xy = np.array([x_actual[0], x_actual[1]], dtype=float)
                actual_yaw = yaw_from_quat(x_actual[3:7])
                actual_vel_xy = np.array([x_actual[nq + 0], x_actual[nq + 1]], dtype=float)
                actual_yaw_rate = float(x_actual[nq + 5]) if x_actual.shape[0] >= nq + 6 else 0.0
                pos_err = actual_pos_xy - desired_pos_xy
                yaw_err = wrap_pi(actual_yaw - desired_yaw)
                vel_err = actual_vel_xy - desired_vel_xy
                yaw_rate_err = actual_yaw_rate - desired_yaw_rate
                print("[MPC] desired/actual base xy+yaw pos/vel")
                print(
                    "  desired_pos_xy={:.6g},{:.6g} desired_yaw={:.6g} desired_vel_xy={:.6g},{:.6g} desired_yaw_rate={:.6g}".format(
                        desired_pos_xy[0],
                        desired_pos_xy[1],
                        desired_yaw,
                        desired_vel_xy[0],
                ...
                print(
                    "  actual_pos_xy={:.6g},{:.6g} actual_yaw={:.6g} actual_vel_xy={:.6g},{:.6g} actual_yaw_rate={:.6g}".format(
                        actual_pos_xy[0],
                        actual_pos_xy[1],
                        actual_yaw,
                        actual_vel_xy[0],
                ...
                print(
                    "  err_pos_xy={:.6g},{:.6g} err_yaw={:.6g} err_vel_xy={:.6g},{:.6g} err_yaw_rate={:.6g}".format(
                        pos_err[0],
                        pos_err[1],
                        yaw_err,
                        vel_err[0],
            ...
            if solve_iters is None:
                print(f"[MPC] solve_time={solve_time * 1000.0:.3f} ms iters=n/a")
            else:
                print(f"[MPC] solve_time={solve_time * 1000.0:.3f} ms iters={solve_iters}")
                ...
                for row in rows:
                    debug_csv_writer.writerow(
                        [
                            ...
                            row.get("hip_sum_res"),
                            row.get("icr_axle_res"),
                        ]
                    )
                debug_csv_file.flush()

        hide_q = model_handler.getReferenceState()[:nq].copy()
        hide_q[0] = 0.0
        hide_q[1] = 0.0
        hide_q[2] = -100.0
        hide_q[3:7] = np.array([0.0, 0.0, 0.0, 1.0])
        for joint_name, qv in zip(joint_order, joints0):
            joint_id = model.getJointId(joint_name)
            idx_q = model.joints[joint_id].idx_q
            hide_q[idx_q : idx_q + model.joints[joint_id].nq] = qv

        if SHOW_MPC_GHOSTS:
            _ensure_ghosts(mpc_terminal_ghosts, len(segment_results), [0.9, 0.8, 0.2, MPC_GHOST_ALPHA])
            for i, seg_res in enumerate(segment_results):
                mpc = seg_res["mpc"]
                if not mpc.xs:
                    continue
                ghost_id, ghost_joints = mpc_terminal_ghosts[i]
            ...
            for i in range(len(segment_results), len(mpc_terminal_ghosts)):
                ghost_id, ghost_joints = mpc_terminal_ghosts[i]
                update_ghost(ghost_id, ghost_joints, hide_q)

        if SHOW_MPC_REF_GHOST:
            _ensure_ghosts(mpc_ref_ghosts, len(segment_results), [0.2, 0.9, 0.2, MPC_GHOST_ALPHA])
            for i, seg_res in enumerate(segment_results):
                mpc = seg_res["mpc"]
                try:
                    x_term_ref = np.asarray(mpc.ocp_handler.getTerminalReferenceState())
                ...
                ghost_id, ghost_joints = mpc_ref_ghosts[i]
                update_ghost(ghost_id, ghost_joints, q_ref_vis)
            for i in range(len(segment_results), len(mpc_ref_ghosts)):
                ghost_id, ghost_joints = mpc_ref_ghosts[i]
                update_ghost(ghost_id, ghost_joints, hide_q)

        if SHOW_MPC_PROCESS_REF_GHOST:
            _ensure_ghosts(mpc_process_ref_ghosts, len(segment_results), [0.2, 0.7, 1.0, MPC_GHOST_ALPHA])
            for i, seg_res in enumerate(segment_results):
                mpc = seg_res["mpc"]
                try:
                    x_ref0 = np.asarray(mpc.ocp_handler.getReferenceState(0))
                    q_ref0 = x_ref0[:nq].copy()
                except Exception:
                    q_ref0 = seg_res["q_term"].copy()
                q_ref0_vis = q_ref0.copy()
                q_ref0_vis[0] += base_xyz[0] + BASE_POS_OFFSET[0]
                q_ref0_vis[1] += base_xyz[1] + BASE_POS_OFFSET[1]
                q_ref0_vis[2] = q_ref0[2] + BASE_POS_OFFSET[2]
                ghost_id, ghost_joints = mpc_process_ref_ghosts[i]
                update_ghost(ghost_id, ghost_joints, q_ref0_vis)
            for i in range(len(segment_results), len(mpc_process_ref_ghosts)):
                ghost_id, ghost_joints = mpc_process_ref_ghosts[i]
                update_ghost(ghost_id, ghost_joints, hide_q)

        if SHOW_TARGET_BALL and target_ball_ids:
            for i, seg_res in enumerate(segment_results):
                q_term = seg_res["q_term"]
                ball_xy = base_xyz0[:2] + np.array([q_term[0], q_term[1]])
                ball_pos = [ball_xy[0] + BASE_POS_OFFSET[0], ball_xy[1] + BASE_POS_OFFSET[1], args.base_height + BASE_POS_OFFSET[2]]
                p.resetBasePositionAndOrientation(target_ball_ids[i], ball_pos, [0, 0, 0, 1])
            for i in range(len(segment_results), len(target_ball_ids)):
                p.resetBasePositionAndOrientation(target_ball_ids[i], [0.0, 0.0, -100.0], [0, 0, 0, 1])

        if SHOW_MPC_TRAJECTORY and segment_results:
    ...
    while True:
        ...
        r_cmd = float(p.readUserDebugParameter(r_cmd_slider))
        segments = compute_segment_plan(r_cmd)
        wz_cmd = float(segments[0]["w_cmd"]) if segments else 0.0
        cost_vals = []
        all_slider_keys = sorted(list(cost_sliders.keys()) + list(jointpos_sliders.keys()))
        for key in all_slider_keys:
            if key in cost_sliders:
        ...
        cmd = (round(r_cmd, 6), round(wz_cmd, 6), *cost_vals)
        if cmd != last_cmd:
            last_cmd = cmd
            process_frame()
